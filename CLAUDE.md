...
2. **별도 Worker `sclm-push-cron`** (`web/push-cron/`) — 매일 08:00·08:10 KST에 **두 엔드포인트를 각각** `X-Cron-Secret`으로 호출. 배포: `cd web/push-cron && npx wrangler deploy`.
   ...
   - **점검 경고(`maybeAlert`)는 메일 + 웹푸시 둘 다로 나간다** (2026-08-07). 점검 메일도 같은 Resend 를 타므로 메일 경로가 통째로 죽으면 정작 그날 경고가 못 나갔다. 하루 한 번 가드(`lastAlertDay`)는 **둘 중 하나라도 닿았을 때만** 세운다 — 먼저 세우면 08:10 재시도가 조용히 사라진다. 구독이 0이면 `sent`도 0이고 그건 '보냈다'가 아니다. 감시 대상은 `collectIssues`가 정하며 **브리핑 메일 실패도 포함**한다(빠져 있어서 아침 메일만 조용히 안 오는 날을 못 잡았다).
