...
2. **별도 Worker `sclm-push-cron`** (`web/push-cron/`) — 매일 08:00·08:10 KST에 **두 엔드포인트를 각각** `X-Cron-Secret`으로 호출. 배포: `cd web/push-cron && npx wrangler deploy`.
   ...
   - **점검 경고(`maybeAlert`)는 메일 + 웹푸시 둘 다로 나간다** (2026-08-07). 점검 메일도 같은 Resend 를 타므로 메일 경로가 통째로 죽으면 정작 그날 경고가 못 나갔다. 하루 한 번 가드(`lastAlertDay`)는 **둘 중 하나라도 닿았을 때만** 세운다 — 먼저 세우면 08:10 재시도가 조용히 사라진다. 구독이 0이면 `sent`도 0이고 그건 '보냈다'가 아니다. 감시 대상은 `collectIssues`가 정하며 **브리핑 메일 실패도 포함**한다(빠져 있어서 아침 메일만 조용히 안 오는 날을 못 잡았다). 카카오 실패 때 따로 쏘던 전용 푸시(`sclm-kakao-fail`)는 **뺐다** — 점검 경고가 같은 얘기를 하게 돼 푸시가 두 번 왔다. 푸시 본문은 암호화돼 나가므로 테스트는 **횟수**로 검증한다.
...
3. **별도 Worker `sclm-email-inbox`** (`web/email-inbox/`, 2026-08-05) — **메일을 전달하면 할 일이 된다.** Cloudflare Email Routing 이 지정 주소로 온 메일을 이 워커에 넘기고, 워커는 `postal-mime` 으로 봉투만 뜯어 Pages 의 `/api/mail-inbox` 로 넘긴다. 배포: `cd web/email-inbox && npm install && npx wrangler deploy` + `npx wrangler secret put MAIL_INBOX_SECRET`.
   ...
   - 🛑 **아웃룩 메일을 Graph 로 직접 읽는 방식은 하지 않기로 했다(2026-08-06 결정).** "전달하지 않아도 자동으로 집어온다"가 유일한 이득인데 대가가 크다. ① `Mail.Read` **위임** 권한은 문서상 관리자 동의 불필요(`No`)지만, **MC1097272**(2025-07~08, 기존 테넌트 전체 적용)로 기본 정책이 Microsoft 관리형으로 바뀌어 **저영향으로 분류된 권한만** 본인 동의가 된다 — `Mail.Read` 는 거기 없으므로 실제로는 관리자 승인 화면이 뜬다. ② 앱 등록 자체도 기업 테넌트에서 흔히 막혀 있다(동의와 별개 설정). ③ 대안인 **아웃룩 규칙 자동전달도** Exchange Online 이 외부 도메인 자동전달을 기본 차단해 걸릴 수 있다(수동 전달은 됨). 즉 **IT 승인 없이 되는 건 지금의 수동 전달뿐**이고, 그 편의를 위해 메일함 전체 읽기 권한을 앱에 주는 건 균형이 안 맞는다. 전달 건수가 하루 열 건씩으로 늘면 그때 다시 볼 것.
