...
```jsonc
{
  ...
  "channels": ["마리오아울렛", "..."],           // 중분류 마스터
  "channelProjects": { "마리오아울렛": "cat-1" }, // 중분류 → 소속 대분류(projectId)
  "subMaster": ["하프", "패플"],                  // 소분류 공용 목록(이름은 한 번만)
  "subChannels": { "마리오아울렛": ["하프"], "대백": ["하프", "패플"] }, // 중분류 → 연결된 소분류
...
#### 분류 체계 — 대분류 > 중분류 > 소분류 (종속형 트리)

2026-07-29 도입. **내부 필드명은 옛 이름 그대로**이고 화면 용어만 3단계로 바꾼 것이라, 기존 데이터를 이전하지 않았다.

| 단계 | 화면 용어 | 필드 | 마스터 | 소속 |
|---|---|---|---|---|
| 1 | 대분류 | `todo.projectId` | `projects[]` | — (칸반·캘린더 색상과 공유) |
| 2 | 중분류 | `todo.channel` | `channels[]` | `channelProjects[중분류] = projectId` |
| 3 | 소분류 | `todo.subChannel` | `subMaster[]` (공용) | `subChannels[중분류][]` 로 **다대다 연결** |

- **마이그레이션**: `migrateTaxonomy(state)`(순수 함수, 앱 로드 시 1회)가 소속 없는 중분류를 **"그 중분류를 쓴 할 일에서 가장 많이 등장한 대분류"** 로 편입한다. 소속 대분류가 삭제되면 다시 추론한다. 검증: `web/tests/taxonomy.test.mjs`.
- **소분류가 공용인 이유**: 같은 소분류를 여러 중분류가 함께 운영한다(예: '하프'를 마리오아울렛·대백 두 점포가 취급). 중분류마다 같은 이름을 다시 등록하게 하면 오타·중복 집계가 생기므로, 이름은 `subMaster`에 한 번만 두고 연결만 늘린다. 삭제도 2단계다 — 중분류에서 **연결 해제**(`unlinkSub`) / 공용 목록에서 **완전 삭제**(`subDeleteGlobal`, 모든 연결이 끊김).
- **관리 화면**: Categories 한 곳에서 3단계 모두 관리한다. **가로 3열**(대분류|중분류|소분류)로, 왼쪽을 고르면 오른쪽이 좁혀지는 방식이다(`catSelProj`·`catSelMid`). 3열은 공용 소분류 전체를 보여주고 **체크박스가 곧 연결/해제**다. 대분류 추가·수정은 Projects 화면과 **같은 모달**(`openProjectModal`)을 재사용하고, 대분류를 지우면 그 소속 중분류는 `default`로 옮겨진다. 중분류는 `openMidModal`에서 이름·소속·색상을 한 번에 고친다.
- **종속 UI**: 할 일 모달·목록 필터 모두 상위 선택에 따라 하위 후보가 좁혀진다. 단 **입력을 막지는 않는다** — 다른 대분류의 중분류도 목록 뒤쪽에 "다른 대분류" 표시로 남겨 두고, 이미 어긋난 값도 지우지 않는다(실데이터를 조용히 잃지 않기 위함).
- **중분류 이동**(Categories 화면)은 그 중분류를 쓰는 **할 일의 대분류까지 함께** 바꾼다. 안 그러면 "대분류 > 중분류" 관계가 깨진다.
- **삭제 규칙**: 마스터 목록에서만 빼고 **할 일에 적힌 값은 남긴다**(중분류·소분류 공통). 값까지 지우면 되돌릴 수 없다.
